...
class OperationsTests(unittest.TestCase):
    ...
    def test_profiles_from_bulk_placeholders_orgs_missing_from_snapshot(self):
        fields = ["organisasjonsnummer", "navn", "organisasjonsform.kode", "sisteInnsendteAarsregnskap", "konkurs", "underAvvikling"]
        rows = [
            {"organisasjonsnummer": "923609016", "navn": "Example AS", "organisasjonsform.kode": "AS", "sisteInnsendteAarsregnskap": "2025", "konkurs": "false", "underAvvikling": "false"},
        ]
        with tempfile.TemporaryDirectory() as directory:
            path = Path(directory) / "registry.csv.gz"
            with gzip.open(path, "wt", encoding="utf-8", newline="") as handle:
                ...
                writer.writeheader()
                writer.writerows(rows)
            profiles, registry_metadata = profiles_from_bulk(path, ["923609016", "999999999"])
        found_profile, missing_profile = profiles
        self.assertEqual(found_profile["organisation_number"], "923609016")
        self.assertEqual(found_profile["evidence"]["registry"]["status"], "available")
        self.assertEqual(missing_profile["organisation_number"], "999999999")
        self.assertEqual(missing_profile["evidence"]["registry"]["status"], "not_found")
